...
- `setSimpleCounterDate(id, date, value)` / `setSimpleCounterToday(id, value)` — absolute-set a day's value for an **existing** counter; throws `Counter {id} not found` if the id doesn't exist yet (confirmed against SP source — it is not an upsert)
- `setCounter(id, value)` — the one method that *does* upsert: creates a default `ClickCounter` (`title: id`, `type: ClickCounter`, `isEnabled: true`) if `getSimpleCounter(id)` returns nothing, otherwise behaves like `setSimpleCounterToday`
...
- No native create call. `setCounter` (above) is the upsert path SP's own plugins use to create a counter implicitly.
...
There's no `addSimpleCounter`. Sequence: (1) call `setCounter(freshId, 0)` — the only method that upserts, creating the counter with defaults (title = id, `type: ClickCounter`, `isEnabled: true`); (2) immediately call `updateSimpleCounter(freshId, { title, icon, isTrackStreaks, streakMode, streakMinValue, streakWeekDays, streakWeeklyFrequency })` to apply the caller's actual config before anything else can observe the placeholder state. The plugin generates the id itself (matching how SP's own `addTag`/`addProject` return a generated id, not a caller-supplied one) rather than accepting a caller-supplied id, to avoid an id-collision failure mode when the "create" is really an upsert against arbitrary existing counters.

**Correction found during manual smoke testing:** the design originally called `setSimpleCounterDate(freshId, today, 0)` for step (1), based on a misreading that conflated it with `setCounter`'s upsert behavior. `setSimpleCounterDate` throws `Counter {id} not found` for a non-existent id — it is not an upsert. `plugin.js` and this doc now correctly use `setCounter`, and `missingHabitApiError()`'s capability probe includes `setCounter` in its required-methods check.
