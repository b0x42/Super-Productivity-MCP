...
- New `plugin/plugin.js` command handlers wired to `PluginAPI.getAllSimpleCounters`, `getSimpleCounter`, `updateSimpleCounter`, `deleteSimpleCounter`, `setSimpleCounterDate`, and `setCounter`. There is no native "create" call — `create_habit` is implemented as upsert-then-configure (`setCounter` on a fresh id to force creation via its upsert behavior, then `updateSimpleCounter` to set title/streak config).
